...
class rp2040_pad_control
    : std::conditional<USE_SIMULATED_REGISTERS,
                       simulated_peripheral<rp2040_pad_control>, void> {
public:
  static constexpr uintptr_t base_address = 0x4001c000;

  ~rp2040_pad_control();
  void operator delete(void *addr) {
    static_cast<rp2040_pad_control *>(addr)->~rp2040_pad_control();
  };
...
private:
    ...
    return reinterpret_cast<rp2040_pad_control *>(base_address);
  }

  static std::weak_ptr<rp2040_pad_control> storage_handle;
  friend std::shared_ptr<rp2040_pad_control>
  std::make_shared<rp2040_pad_control>();
