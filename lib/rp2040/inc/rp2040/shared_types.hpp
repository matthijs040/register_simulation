...
using type =
    std::conditional_t<mock, simulated_device_register<field_type>, field_type>;

template <typename bitstate, std::size_t offset, std::size_t num_bits>
using field = bitfield<bitstate, offset, num_bits, reg::type<bitfield<bitstate, offset, num_bits, uint32_t>>>;
